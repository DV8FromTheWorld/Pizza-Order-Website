...
function custInfoList() {
  ...
  form +=  '<div class="input-group form-group"><input id="phoneCheck" type="text" class="form-control bfh-phone" name="phone" placeholder="(999) 999-9999" data-format="(ddd) ddd-dddd" maxlength="10" required></div>';
...
$(document).on('input', 'input', function() {
	...
	case 'emailCheck':
	    ...
	    break;
	case 'phoneCheck':
	    var phoneTest = /^\d\d\d\d\d\d\d\d\d\d/.test(this.value);
	    $(this).parent().toggleClass('has-success', phoneTest);
	    $(this).parent().toggleClass('has-error', !phoneTest);
	    break;
	default:
	    alert('not so tu madre naow');
	    break;
	}
